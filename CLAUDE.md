...
- **Frontend testleri:** `cd frontend && npm test` (Vitest). Kapsam bilinçli olarak saf mantık ve sunucu kodu: yükleme kısıtları, arka plan kaldırma vekili, zemin vekili, imzalı URL yenileme zamanlaması ve kompozisyon geometrisi. Bileşen testleri (React Testing Library) ve E2E (Playwright) bilinçli olarak Faz 7'ye bırakıldı.
